...
router.post('/register', async (req, res) => {
    try {
        ...
        const verificationToken = jwtUtil_1.JWTUtils.generateVerificationToken({
...
router.post('/login', async (req, res) => {
    try {
        ...
        await user.save();
        // Tạo JWT token
        const accessToken = jwtUtil_1.JWTUtils.generateAccessToken({
            userId: user._id.toString(),
 ...
 */
router.get('/me', jwtMiddleware_1.authenticateToken, async (req, res) => {
    try {
...
router.post('/verify-email', async (req, res) => {
    try {
        ...
        const decoded = jwtUtil_1.JWTUtils.verifyVerificationToken(token);
        if (!decoded) {
            return res.status(400).json({
                success: false,
...
router.post('/forgot-password', async (req, res) => {
    try {
        ...
        const resetToken = jwtUtil_1.JWTUtils.generateResetPasswordToken({
            email: user.email,
            userId: user._id.toString()
        });
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        const decoded = jwtUtil_1.JWTUtils.verifyResetPasswordToken(token);
        if (!decoded) {
            return res.status(400).json({
                success: false,
                message: 'Token không hợp lệ hoặc đã hết hạn'
 ...
 */
router.post('/change-password', jwtMiddleware_1.authenticateToken, async (req, res) => {
    try {
 ...
 */
router.post('/logout', jwtMiddleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.jwtUser?.userId;
        if (userId) {
